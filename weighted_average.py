#此代码先将50张图片输出提取网络，然后将得出的残差输入加权平均网络，根据提取噪声残差的质量进行加权。
import os
import argparse
import time
import h5py
...
#init_validate_dataset
    ...
    for epoch in rangeepochs:
        ...
        for i in range(numofbatch):
            ...
            ref = module_fusion(out_img)

            loss = mloss(ref, PRNUB)
            loss.backward()
        ...
        if (epoch + 1) % 1 == 0:
            torch.save(module_fusion.state_dict(),os.path.join(opt.outf, f'residual_fusion-epoch{epoch + 1}.pth'))
        ...
        for i in range(num):
            ...
            with torch.no_grad():
                ...
                PRNUB1=moudle_dncnn(PRNUB)
                PRNUV = module_fusion(PRNUB1)
        ...
        if OUR_prnu_auc > max_AUC:
            ...
            moudle_name = 'residual_fusion' + '_' + str(epoch + 1) + '_' + str(max_AUC) + '.pth'
    ...
    torch.save(module_fusion.state_dict(), os.path.join(opt.outf, 'residual_fusion_end.pth'))
