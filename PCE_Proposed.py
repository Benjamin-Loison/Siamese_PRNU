...
def main():
    ...
    #
    # # ADNet
    moudle_fusion=ImageFusionNet()
    moudle_fusion.cuda()
    moudle_fusion.load_state_dict(torch.load('logs_nat/ImageFusion-Net*image-softmax-epoch_161_0.9271527777777778.pth'))
    model_ADNet = DnCNN(channels=1)
    model_ADNet.cuda()
    # pretrained_model = torch.load('logs_nat/Siamese_PRNU_distance_0.5_8_27.047347688343216.pth')
    # pretrained_model = torch.load('logs_nat/Siamese_distance_100image _34_AUC=0.9224090576171875.pth')
    # pretrained_model = torch.load('logs_nat/Siamese_mloss_fine_tune+_1_AUC=0.956.pth')
    # pretrained_model = torch.load('logs_nat/Siamese_mloss_45_AUC=0.9009759999999999.pth')
    # pretrained_model = torch.load('logs_nat_taishi/Siamese_mloss_1_AUC=0.5910491943359375.pth')
    ...
    sio.savemat('Dresden_PCE_dict_256/dude_PCE_two.mat', {'intra_pce': adnet_intra_pce, 'inter_pce': adnet_inter_pce})

    ADNet_PCE = sio.loadmat('Dresden_PCE_dict_256/dude_PCE_two.mat')
