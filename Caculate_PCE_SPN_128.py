...
def main():
    ...
    pretrained_model = torch.load('../pre_train_model/DnCNN_SPN_MSE.pth')
    ...
    for i in range(11):
        ...
        for j in range(int(camera_model[-2])):  #将字符串 camera_model 的倒数第二个字符（通常是相机模型的编号）转换为整数
            ...
            ## 2 read the RP image
            RPname = DID_files[k]
            RP = cv2.imread(RPname, cv2.IMREAD_GRAYSCALE)  #转换为灰度图像
    ...
    sio.savemat('../Dresen_PCE_dict_128/SPN_PCE.mat', {'intra_pce': adnet_intra_pce, 'inter_pce': adnet_inter_pce})

    ADNet_PCE = sio.loadmat('../Dresen_PCE_dict_128/SPN_PCE.mat')
