...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', type=int, default=20)  # minim um size of the image
    parser.add_argument('--crop_size', type=int, default=20)  # size of the central patch, to be cropped
    parser.add_argument('--random_cropping', type=int, default=40)  # 40 pixel for random cropping during training
    parser.add_argument('--model_dir', type=str, default='./weight_Eff_repB0_dresden/Pixle_256_256-dwt_shift11111111111')  # directory with CNN weights -101为cross2 -201为cross 601wei dresden
    parser.add_argument('--gpu', type=str, default='0')  # gpu to be used
    parser.add_argument('--num_epochs_train', type=int, default=200)  # number of training epochs
    ...
    logger.info(config.base_network + '_' + str(config.image_size) + '_' + str(config.crop_size)+'_dwt_shift111111')
    logger.info("Preparing train data loader")
    #
    train_prnu = [load_prnu(item, config.image_size)[None, :, :] for item in list_dev_train]

    first_dim_length = len(train_prnu)
    print(f"train_prnu 列表的元素数量（第一个维度长度）: {first_dim_length}")
    ...
    ##spncnn提取的测试PRNU
    # train_data = [load_res(np.load('/home/seamus20/matlab_wyl/Train_nat_path_Dresden_40cameras_SPNCNN/train/%s.npy' % item).tolist(), config.image_size) for item in list_dev_train]
    ...
    valid_data = [load_res(np.load('/home/seamus20/matlab_wyl/Train_nat_path_Dresden_40cameras_DWT/valid/%s.npy' % item).tolist(), config.image_size) for item in
                  list_dev_valid]
    # #spncnn
    # valid_data = [load_res(np.load('/home/seamus20/matlab_wyl/Train_nat_path_Dresden_40cameras_SPNCNN/valid/%s.npy' % item).tolist(), config.image_size) for item in
    #               list_dev_valid]
