...
def NoiseExtract(Im,qmf,sigma,L):
    ...
    nr = (np.ceil((M+minpad)/m)*m).astype(np.int64);  nc = (np.ceil((N+minpad)/m)*m).astype(np.int64)  # dimensions of the padded image (always pad 8 pixels or more)
    pr = np.ceil((nr-M)/2).astype(np.int64)      # number of padded rows on the top
    prd= np.floor((nr-M)/2).astype(np.int64)    # number of padded rows at the bottom
    pc = np.ceil((nc-N)/2).astype(np.int64)      # number of padded columns on the left
    pcr= np.floor((nc-N)/2).astype(np.int64)     # number of padded columns on the right
    Im = np.block([
