...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_file', type=str, default='./output_Pce/results_256_256-dre-dwt_shift50.npz')  # output file with result
    ...
    for indexD in range(num_dev):  # loop on devices
        ...
        for indexR in tqdm(range(num_residuals)):  # loop on the residuals of the device
            ...
            for prnuD in range(num_dev):
                ...
                data_shift_range[prnuD] = max([list_prnu[prnuD,:,:].shape[0] - res_processed.shape[0], list_prnu[prnuD,:,:].shape[1] - res_processed.shape[1]], [50, 50])
