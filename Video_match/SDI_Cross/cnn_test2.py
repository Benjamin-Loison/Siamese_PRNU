...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=str, default='./weight_Eff_repB0/Pixle_256_180')  # directory with CNN weights
    parser.add_argument('--output_file', type=str, default='./output_Eff_repB0_shift/results_test1.npz')  # output file with result
    ...
    for indexD in range(num_dev):  # loop on devices
        ...
        for indexR in tqdm(range(num_residuals)):  # loop on the residuals of the device
            ...
            for prnuD in range(num_dev):
                data[prnuD,:,:] = crosscorr2(preprocessing_function(list_prnu[prnuD,:,:].astype(np.float32)), res_processed)
            ...
            score_mat[indexD][indexR][:, 0] = result[:, 1]
