...
class DataGenerator(Dataset):
    def __init__(self, batch_size, list_data0, list_data1,
        ...
        # Set info
        self.size_block = int(size_block)  # 2
        self.num_blocks = int(num_blocks)  # 18   16  dresden=20
        self.to_categorical = int(to_categorical)  # 2
        self.batch_size = int(batch_size)  # 72  80
        self.num_combine = self.size_block * self.num_blocks  # 36  40
        self.list_data0 = list_data0
        # self.list_data0 = list_data0
        self.list_data1 = list_data1
        self.num_classes = len(self.list_data0)  # 36  40
        self.step_blocks = self.num_classes // self.num_blocks  # 2
        ...
        self.data_shape0 = self.list_data0[0].shape[1]
        self.data_shape1 = self.list_data0[0].shape[2]
        self.num_samples = 0
        ...
        for index in range(self.num_classes):
            ...
            assert (self.list_data0[index].shape[1] == self.data_shape0)  # same num of rows in list_data0
            assert (self.list_data0[index].shape[2] == self.data_shape1)  # same num of cols in list_data0
            assert (self.list_data1[index].shape[1] == self.data_shape0)  # same num of rows in list_data1
            assert (self.list_data1[index].shape[2] == self.data_shape1)  # same num of cols in list_data1
            self.num_samples = max(self.num_samples, self.list_data0[index].shape[0])
            self.num_samples = max(self.num_samples, self.list_data1[index].shape[0])
            print(index, self.list_data0[index].shape[0], self.list_data1[index].shape[0])

        self.batch_size_for_combine = self.batch_size // self.num_combine  # 2
        ...
        # Data for iteration
        self._indices0 = [np.arange(0, self.list_data0[index].shape[0]) for index in range(self.num_classes)]
        self._indices1 = [np.arange(0, self.list_data1[index].shape[0]) for index in range(self.num_classes)]
    ...
    def print_info(self, logger):
        ...
        logger.info(self.num_batches)
        logger.info(self.num_blocks)
        logger.info(self.size_block)
    ...
    def __getitem__(self, index):
        ...
        for k in range(self.batch_size_for_combine):
            ...
            for indexCa in range(self.num_blocks):
                ...
                img1A = self.list_data1[cA][index1A, crop_0:(crop_0 + self.size_crop), crop_1:(crop_1 + self.size_crop)]
                ...
                for indexCb in range(self.size_block):
                    ...
                    img0B = self.list_data0[cB][index0B, :, :]
                    ...
                    C = preprocessing_function(crosscorr2(img0B,img1A))
                    #
                    # C = crosscorr2(img0B,img1A)

                    X_batch.append(C[None,:,:])
        ...
        if self.to_categorical > 1:
            Y_batch = to_categorical(Y_batch, self.to_categorical)
