...
def PRNU_Extract(Camera_dir_name, model_dncnn, model_fusion, tile_size):
    ...
    # image_path_pattern = os.path.join('/home/seamus20/matlab_wyl/Camera_Dresden', Camera_dir_name,  '*.JPG')
    image_path_pattern = os.path.join('/home/seamus20/VISION', Camera_dir_name, 'images', 'flat', '*.jpg')
    # image_path_pattern = os.path.join('/home/seamus20/matlab_wyl/VISION_buchong', Camera_dir_name,  '*.jpg')
    ...
    for tensor in tensor_list:
        ...
        with torch.no_grad():
            ...
            # #零均值化操作
            # mean_value = torch.mean(PRNU_PATCH)
            mean_value = torch.mean(PRNU_PATCH, dim=(2, 3))
            PRNU_PATCH = PRNU_PATCH - mean_value
...
def main():
    ...
    module_fusion.cuda()
    module_fusion.load_state_dict(torch.load('weights/conv5_11_0.9290277777777779.pth'))
    #
    base_dir = '/home/seamus20/VISION'
    # base_dir = '/home/seamus20/matlab_wyl/VISION_buchong'
    ...
    # vision 图像有问题的相机索引：1图像大小不一样,22图像读取错误,30图像为纵向,32没有图像
    ...
    for Camera_dir_name in Cameras_dir_sorted:
        ...
        #npy格式
        save_path = os.path.join('/home/seamus20/matlab_wyl', os.path.basename(Camera_dir_name)+ '.npy')
