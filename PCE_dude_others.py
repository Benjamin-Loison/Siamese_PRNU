...
def main():
    ...
    # pretrained_model = torch.load('logs_nat/Siamese_mloss_fine_tune+_1_AUC=0.956.pth')
    ...
    k = 0
    root_dir = '/home/seamus20/Dresden'
    camera_models = ['Agfa_DC-504_0(1)', 'Agfa_DC-733s_0(1)', 'Agfa_DC-830i_0(1)', 'Canon_Ixus55_0(1)',
                     'Canon_PowerShotA640_0(1)']
    DID_files = sorted(glob.glob(os.path.join('Dresden_others_PRNU',  '*.mat')))
    for i in range(5):
        ...
        for j in range(int(camera_model[-2])):  #将字符串 camera_model 的倒数第二个字符（通常是相机模型的编号）转换为整数
            ...
            # 负样本（类间）
            r = list(range(0,i)) + list(range(i+1, 5))   #生成一个不包括i的0-10之间的列表
            camera_model_idx_list = random.sample(r,2)   #列表中选择3个不同的相机模型索引作为负样本

            for m in range(2):
                print('+++++++++++++++++++++++++++')
                print(m)
                inter_img_num = 0
                camera_name = camera_models[camera_model_idx_list[m]][:-3]
                inter_img_files = sorted(glob.glob(os.path.join(root_dir, camera_models[camera_model_idx_list[m]], camera_name, '*.JPG')),reverse=True)
    ...
    sio.savemat('Dresden_PCE_dict_128/Proposed_PCE_one_others1111.mat', {'intra_pce': adnet_intra_pce, 'inter_pce': adnet_inter_pce})

    ADNet_PCE = sio.loadmat('Dresden_PCE_dict_128/Proposed_PCE_one_others1111.mat')
