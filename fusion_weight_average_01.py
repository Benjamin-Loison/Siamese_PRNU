...
from encoder_decoder import QualityWeightNet
import random
from utils import *
...
def main():
    ...
    # data = h5py.File('/home/seamus20/matlab_wyl/Train_Dataset/VISION_train_new_40_50_2810850.mat', 'r')#large new
...
#init_validate_dataset
    ...
    module_fusion = QualityWeightNet()
    module_fusion.cuda()
    # module_fusion.load_state_dict(torch.load('logs_nat/ImageFusion-Net*image-softmax-epoch_161_0.9271527777777778.pth'))
    # module_fusion.load_state_dict(torch.load('logs_nat/weight_average_4_0.5831944444444443.pth'))
    ...
    for epoch in rangeepochs:
        ...
        if (epoch + 1) % 1 == 0:
            torch.save(module_fusion.state_dict(),os.path.join(opt.outf, f'weight_average_encoder-nowiener-epoch{epoch + 1}.pth'))
        ...
        if OUR_prnu_auc > max_AUC:
            ...
            moudle_name = 'weight_average-encoder-nowiener' + '_' + str(epoch + 1) + '_' + str(max_AUC) + '.pth'
    ...
    torch.save(module_fusion.state_dict(), os.path.join(opt.outf, 'weight_average-encoder-nowiener.pth'))
