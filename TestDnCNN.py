...
def main():
    ...
    # data_train = h5py.File('data/VISIONdemo_40_64.mat', 'r')
    # data_test = h5py.File('data/VISIONval_128_1.mat', 'r')
    # # PRNU = np.array(data_train['PRNU'])
    # # img = np.array(data_train['inputs'])
    # PRNUV = np.array(data_test['PRNUV'])
    # imgV = np.array(data_test['inputsV'])
    ...
    #load pth
    model.load_state_dict(torch.load('logs_nat/Siamese_PRNU_0.03_7_97.5136333007283.pth'))
    # model.load_state_dict(torch.load('logs_nat/dude_nat_70.pth'))
    # model.load_state_dict(torch.load('logs_nat/Siamese_PRNU_0.0085_lr=e-5__12_94.016804213289_20.357848310119916.pth'))
    # Move to GPU
    ...
    # avg_pce = 0
    #
    # imgV1 = Variable(torch.FloatTensor(imgV).cuda())
    # PRNUV1 = Variable(torch.FloatTensor(PRNUV).cuda())
    # with torch.no_grad():
    #     EPRNU = model(imgV1)
    #     PRNUV1 = model(PRNUV1)
    #
    ...
    #
    # EPRNU = EPRNU.squeeze().float().cpu().numpy()
    # PRNUV1 = PRNUV1.squeeze().float().cpu().numpy()
    # imgV1 = imgV1.squeeze().float().cpu().numpy()
    # for i in range(100):
    #     select_EPRNU = EPRNU[i, :, :]
    #     select_PRNUV = PRNUV1[i, :, :]
    #     select_imgV = imgV1[i, :, :]
    #     # -----------------------
    #     # calculate PCE
    #     # -----------------------
    #     KI = select_imgV * select_PRNUV
    #     C = crosscorr(select_EPRNU, KI)
    #     PCE = PCE1(C)
    #     print(f"PCE for block {i + 1}: {PCE}")
    #     avg_pce += PCE
