...
def main():
    ...
    # pretrained_model = torch.load('logs_nat/Siamese_euc_fine tune_1_AUC=0.6596527099609375.pth')
    # pretrained_model = torch.load('logs_nat/Siamese_mloss_1_AUC=0.46675200000000006.pth')
    # pretrained_model = torch.load('logs_nat/Siamese_euc_fine tune_10+_5_AUC=0.786041259765625.pth')
    # pretrained_model = torch.load('logs_nat/Siamese_euclidean+_3_AUC=0.401424.pth')
    ...
    DID_files = sorted(glob.glob(os.path.join('Camera_Dresden_PRNU',  '*.mat')))
    for i in range(11):
        ...
        for j in range(int(camera_model[-2])):  #将字符串 camera_model 的倒数第二个字符（通常是相机模型的编号）转换为整数
            ...
            img_files = sorted(glob.glob(os.path.join(root_dir, camera_model, camera_name, '*.JPG')), reverse=True) #倒序，因为前面100张图像用于估计参考PRNU
    ...
    sio.savemat('Dresden_PCE_dict_128/Proposed_PCE_one111111.mat', {'intra_pce': adnet_intra_pce, 'inter_pce': adnet_inter_pce})

    ADNet_PCE = sio.loadmat('Dresden_PCE_dict_128/Proposed_PCE_one111111.mat')
