...
def main():
    ...
    data = h5py.File('/home/seamus20/matlab_wyl/Train_fusion_dude/VISION_fusion-dude_train_40_50_5429000.mat', 'r')#
    PRNU = np.array(data['IMG'])
    img = np.array(data['inputs'])
    PRNU_ref = np.array(data['PRNU'])

    data_pos = h5py.File('Dresden_test/DRESDEN_TEST_POS128_256_120.mat', 'r')
    img_f = np.array(data_pos['inputs'])
    # img_f = np.repeat(img_f, 20, axis=0)
...
#init_validate_dataset
    ...
    moudle_dncnn.load_state_dict(torch.load('logs_nat/ImageFusion-dude-epoch30.pth'))
    ...
    for param in module_fusion.parameters():
    ...
    for epoch in rangeepochs:
        ...
        for i in range(numofbatch):
            ...
            PRNUR = PRNU_ref[PRNUBbegin:PRNUBend, :, :, :]
            ...
            temptrans = np.transpose(PRNUR, (2, 3, 0, 1))
            temptrans = data_augmentation(temptrans, augmod)
            PRNUR = np.transpose(temptrans, (2, 3, 0, 1)).copy()

            #把训练数据放到gpu上
            ImgB = Variable(torch.FloatTensor(ImgB).cuda())
            PRNUB = Variable(torch.FloatTensor(PRNUB).cuda())
            PRNUR = Variable(torch.FloatTensor(PRNUR).cuda())

            # siamese network
            out_img = module_fusion(ImgB)
            ref = moudle_dncnn(out_img)
            test= moudle_dncnn(PRNUB)
            # ref1=(ref*0+PRNUR*1)
            # loss = mloss(ref1, test)

            distances = calculate_distance(ref, test)
            probabilities = calculate_probabilities(distances)
            batch_loss = calculate_batch_loss(probabilities, group_indices)
            batch_loss_tensor = batch_loss
            total_loss = batch_loss_tensor
        ...
        if (epoch + 1) % 1 == 0:
            torch.save(moudle_dncnn.state_dict(),os.path.join(opt.outf, f'ImageFusion-dude-epoch{epoch + 1}.pth'))
        ...
        #     moudle_name = 'ImageFusion-Net*image-norm-epoch' + '_' + str(epoch + 1) + '_' + str(max_AUC) + '.pth'
    ...
    torch.save(module_fusion.state_dict(), os.path.join(opt.outf, 'ImageFusion-dude.pth'))
