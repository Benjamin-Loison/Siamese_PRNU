...
from moudle_fusion import ImageFusionNet
import random
from utils import *
...
#init_validate_dataset
    ...
    for epoch in rangeepochs:
        ...
        if (epoch + 1) % 25 == 0:
            torch.save(module_fusion.state_dict(),os.path.join(opt.outf, f'ImageFusion-Net*image-5kernel-epoch{epoch + 1}.pth'))
        ...
        if OUR_prnu_auc > max_AUC:
            ...
            moudle_name = 'ImageFusion-Net*image-5kernel-epoch' + '_' + str(epoch + 1) + '_' + str(max_AUC) + '.pth'
    ...
    torch.save(module_fusion.state_dict(), os.path.join(opt.outf, 'ImageFusion-Net*image—5kernel-softmax.pth'))
