...
##论文第四章最后一个相机源识别实验测试代码，测试所提方法+相似性得分计算网络
...
#test-dataset
    ...
    # data = h5py.File('Fusion_test_nowiener/DRESDEN_TEST_LARGE_POSITIVE-CENTER-nowiener128_1_4000.mat', 'r')  # large
    data = h5py.File('Fusion_test_nowiener/DRESDEN_upper-left_POSITIVE-CENTER-nowiener64_1_4000.mat', 'r')  # large
   ...
   #negativel
    ...
    # data1 = h5py.File('Fusion_test_nowiener/DRESDEN_TEST_LARGE_NEGATIVE-CENTER-nowiener128_1_4000.mat', 'r')  # large
    data1 = h5py.File('Fusion_test_nowiener/DRESDEN_upper-left_NEGATIVE-CENTER-nowiener64_1_4000.mat', 'r')  # large
...
#                              drop_last=False, pin_memory=True)
    ...
    # module_fusion.load_state_dict(torch.load('Fusion_Weights/conv5_senet*out_38_0.9319444444444444.pth'))
    # module_fusion.load_state_dict(torch.load('Fusion_Weights/conv5_senet_3_0.9291666666666666.pth'))
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_dir = '/home/seamus20/z_Project/Video_match/weight_Eff_repB0_dresden/Pixle_128_128-fusion_1'
...
#positive_fusion
    ...
    for i in range(num):
        ...
        # 将当前 PCE 值添加到数组中
        pos_pce_fusion.append(score1)
        avg_pce += score1
        # print(f"avg_pce= {avg_pce}")
        print(f"FUSION score for block {idx}: {score1}")
    avg_pce = avg_pce / idx
...
#negative_fusion
    ...
    for i in range(num):
        ...
        score = softmax(result)[:, 1]
        score1 = score.item()
        neg_pce_fusion.append(score1)
        avg_pce += score1
        print(f"FUSION NE PCE for block {idx}: {score1}")
    avg_pce = avg_pce / idx
    print("\n pce_fusion_val: %.6f" % (avg_pce))
...
# # #保存权重用于后续ROC曲线绘制
#     save_path = os.path.join(save_folder, 'Dresden_PRNU-nowiener-center_96_pce_values_upper-right.mat')
#     sio.savemat(save_path, {'positive_pce': pos_pce_values,'negative_pce': neg_pce_values})
#
    save_path = os.path.join(save_folder, 'Dresden_FUSION_64_REP_scores_upper_left.mat')
