...
def crop_image(image):
    ...
    patch3 = image[centerY: centerY+B, centerX - B: centerX]
    patch4 = image[centerY: centerY+B, centerX: centerX + B]
...
def main():
    ...
    pretrained_model = torch.load('../pre_train_model/dude_nat_70.pth')
    model_ADNet.load_state_dict(pretrained_model)
    model_ADNet.eval()
    model_ADNet = model_ADNet.to(device)
    ...
    DID_files = sorted(glob.glob(os.path.join('../quantized_PRNU', 'DID', '*.png')))
    for i in range(11):
        camera_model = camera_models[i]
        for j in range(int(camera_model[-2])):
            img_num = 0
            ## 2 read the RP image
            RPname = DID_files[k]
            ...
            camera_name = camera_model[:-3] + '_' + str(j)
            ...
            while img_num < 100 :
                ...
                _, NoisexnpADNet1, _ = NoiseExtractDL(img_patch1, model_ADNet, postprocess,True)
                ...
                _, NoisexnpADNet2, _ = NoiseExtractDL(img_patch2, model_ADNet, postprocess,True)
                ...
                _, NoisexnpADNet3, _ = NoiseExtractDL(img_patch3, model_ADNet, postprocess,True)
                ...
                _, NoisexnpADNet4, _ = NoiseExtractDL(img_patch4, model_ADNet, postprocess,True)

                ## 5 Compute the correlation between probe and RP
                KI_1 = imgfloat_patch1 * RP_patch1
                ...
                KI_2 = imgfloat_patch2 * RP_patch2
                ...
                KI_3 = imgfloat_patch3 * RP_patch3
                ...
                KI_4 = imgfloat_patch4 * RP_patch4
                ...
                img_num = img_num + 1
                print('Processing intra ---- {}/100:'.format(img_num) + camera_name)
            ...
            # 负样本（类间）
            r = list(range(0,i)) + list(range(i+1, 11))
            camera_model_idx_list = random.sample(r,3)
            ...
            for m in range(3):
                ...
                inter_img_files = sorted(glob.glob(os.path.join(root_dir, camera_models[camera_model_idx_list[m]], camera_name, '*.JPG')))
                if len(inter_img_files) == 0:
                    print('========空list'+camera_models[camera_model_idx_list[m]], camera_name)
                while inter_img_num < 100 :
                    ...
                    print('Processing inter ---- {}/2, {}/100:'.format(m,inter_img_num) + camera_name )
                    # print('PCE_value_FFDNet:')
                    # print(PCE_value_FFDNet1, PCE_value_FFDNet2, PCE_value_FFDNet3, PCE_value_FFDNet4)
                    print('PCE_value_ADNet:')
                    print(PCE_value_ADNet1, PCE_value_ADNet2, PCE_value_ADNet3, PCE_value_ADNet4)
    ...
    sio.savemat('../Dresen_PCE_dict_96/dude_nat_PCE.mat', {'intra_pce': adnet_intra_pce, 'inter_pce': adnet_inter_pce})

    ADNet_PCE = sio.loadmat('../Dresen_PCE_dict_96/dude_nat_PCE.mat')
